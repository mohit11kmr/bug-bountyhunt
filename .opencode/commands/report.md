---
description: Generate a professional bug bounty report formatted for HackerOne submission. Usage: /report
---

# Generate Bug Bounty Report

Generate a complete, submission-ready bug bounty report formatted for HackerOne.

**⛔ NEVER call any HackerOne MCP submission tool** (`submit_report`, `create_report_intent`,
`submit_report_intent`, `update_report_intent`, or any other `mcp__hackerone__*`/`hackerone-mcp`
write action), even if one is available in this session's toolset. This command only ever
writes a local Markdown file — the human submits it manually via the HackerOne web UI.

## Instructions

### Step 0 — Get program handle

Read CLAUDE.md and extract the handle from its header (`HackerOne: https://hackerone.com/<handle>`).
...
Write a complete report using this structure, matching HackerOne's submission form:
...
═══════════════════════════════════════════
 Submit at: https://hackerone.com/<handle>/reports/new
═══════════════════════════════════════════
```
