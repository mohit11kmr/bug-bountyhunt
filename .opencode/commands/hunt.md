...
No hunter or the validator may ever call a HackerOne MCP submission tool (`submit_report`,
`create_report_intent`, `submit_report_intent`, `update_report_intent`, or any other
`mcp__hackerone__*`/`hackerone-mcp` write action), even if one is available in this session's
toolset. Findings only ever get written to `sessions/` and reported to the user — submission
happens later via `/report` (which itself never auto-submits) and a manual human step.
