...
OPENCODE_AGENTS_DIR = Path(__file__).parent / ".opencode" / "agents"
MAX_REPORTS = 40
...
def save_skill(vuln_name: str, content: str) -> Path:
    ...
    print(f"  [+] -> {path}")
    sync_opencode_agent(vuln_name, content)
    return path


def sync_opencode_agent(vuln_name: str, content: str) -> None:
    """Mirror a .claude/skills SKILL.md into .opencode/agents/find-<name>.md
    so the two toolchains don't drift apart (adds `mode: subagent` to frontmatter)."""
    if not content.startswith("---\n"):
        print(f"  [!] Skipping .opencode sync for {vuln_name}: no frontmatter found")
        return

    end = content.find("\n---\n", 4)
    if end == -1:
        print(f"  [!] Skipping .opencode sync for {vuln_name}: malformed frontmatter")
        return
    ...
    agent_path = OPENCODE_AGENTS_DIR / f"find-{vuln_name}.md"
    agent_path.write_text(agent_content)
    print(f"  [+] -> {agent_path} (opencode sync)")
