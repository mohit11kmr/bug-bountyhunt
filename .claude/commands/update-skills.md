...
PROGRAM_SLUG=$(grep -oP 'hackerone\.com/\K[^/\s]+' CLAUDE.md | head -1)
...
### Step 4b — Sync to .opencode/agents (keep both toolchains in sync)

For each skill updated in Step 4, mirror it to `.opencode/agents/find-<name>.md` so opencode and Claude Code stay in sync:

```bash
cp .claude/skills/find-<name>/SKILL.md .opencode/agents/find-<name>.md
```

Then add `mode: subagent` to the copied file's frontmatter (between `description:` and the closing `---`) if it's not already there — the `.opencode/agents/` files need this field, `.claude/skills/` files don't.
